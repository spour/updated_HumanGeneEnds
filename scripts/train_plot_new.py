...
import glob
pos_train  = pd.read_csv(args.positive_training_csv, sep = "\t", header = None, index_col=0).to_numpy()
neg_train = pd.read_csv(args.negative_training_csv, sep = "\t", header = None, index_col=0).to_numpy()
pos_test = pd.read_csv(args.positive_testing_csv, sep = "\t", header = None, index_col=0).to_numpy()
neg_test = pd.read_csv(args.negative_testing_csv, sep = "\t", header = None, index_col=0).to_numpy()
...
print("17032022: pos_train_balanced_shape: ", pos_train_all_class.shape, "pos_test_balanced_shape: ",pos_test_all_class.shape, "neg_train_balanced_shape:",subsample_neg_train.shape, "neg_test_balanced.shape: ",subsample_neg_test.shape)
...
import joblib
joblib.dump(clf_balanced, "RF_17032022_balanced_chr_split_unbal_test.joblib", compress=3)
...
#AUROC
fpr, tpr, threshold_auc_bal = roc_curve(y_test, preds)
roc_auc = auc(fpr, tpr)
...
#AUPRC
precision, recall, thresholds_auprc_bal  = precision_recall_curve(y_test, preds)
...
plt.tight_layout()
plt.savefig('Balanced_RF_17032022_chrsplit.png', bbox_inches='tight', dpi = 300)
plt.clf()

#sAVE metrics
df_balanced = pd.concat([pd.DataFrame(fpr), pd.DataFrame(tpr), pd.DataFrame(precision), pd.DataFrame(recall), pd.DataFrame([roc_auc]), pd.DataFrame([aurpc]), pd.DataFrame([thresholds_auprc_bal]), pd.DataFrame([threshold_auc_bal])], axis=1 )
df_balanced.columns = ['balanced_fpr', 'balanced_tpr', 'balanced_prec', 'balanced_rec', 'balanced_roc', 'balanced_auprc', 'balanced_thresh_auprc','balanced_thresh_auroc']
...
#np.savetxt("classes_data_training_from_pq_joined", classes_data_joined, delimiter="\t")
# X_train, X_test, y_train, y_test = train_test_split(    data, classes, test_size=0.2, random_state=42, stratify=classes)
clf_unablanced  = RandomForestClassifier(n_jobs=-1, class_weight="balanced", n_estimators=30000, min_samples_split=5) #n_estimators=30000, min_samples_split=5
...
import joblib
joblib.dump(clf_unablanced, "RF_17032022_unbalanced_chr_split_IGNORE_wasdonetogetcorrectTHRESHOFRFmodel.joblib", compress=3)
...
figure.set_size_inches(10, 10)
#AUROC
fpr, tpr, threshold_auc_imb = roc_curve(y_test, preds)
roc_auc = auc(fpr, tpr)
print(fpr, tpr, roc_auc)
axs[0].set_title('Receiver Operating Characteristic')
axs[0].plot(fpr, tpr, 'b', label = 'AUC = %0.2f' % roc_auc)
...
#AUPRC
precision, recall, thresholds_auprc_imb  = precision_recall_curve(y_test, preds)
#precision, recall, thresholds = precision_recall_curve(testy, yhat)
# convert to f score
fscore = (2 * precision * recall) / (precision + recall)
# locate the index of the largest f score
ix = np.argmax(fscore)
print('Best Threshold=%f, F-Score=%.3f' % (thresholds_auprc_imb[ix], fscore[ix]))
print(recall[ix], precision[ix], "the recall and precision, 18032022"
)
with open('fscore.txt', 'a') as the_file:
    the_file.write('Best Threshold=%f, F-Score=%.3f' % (thresholds_auprc_imb[ix], fscore[ix]))
    the_file.write('recall=%f, precision=%.3f' % (recall[ix], precision[ix]))
#    the_file.write(recall[ix], precision[ix], "__the recall and precision, 18032022")
aurpc = average_precision_score(y_test, preds)
axs[1].set_title('AUPRC')
axs[1].plot(recall, precision, 'b', label = 'AUPRC = %0.2f' % aurpc)
axs[1].legend(loc = 'lower right')
...
plt.tight_layout()
#SAVE
plt.savefig('Unalanced_RF_17032022_chrsplit.png', bbox_inches='tight', dpi = 300)
plt.clf()

df_unbalanced = pd.concat([pd.DataFrame(fpr), pd.DataFrame(tpr), pd.DataFrame(precision), pd.DataFrame(recall), pd.DataFrame([roc_auc]), pd.DataFrame([aurpc]), pd.DataFrame([threshold_auc_imb]), pd.DataFrame([thresholds_auprc_imb])], axis=1 )
df_unbalanced.columns = ['unbalanced_fpr', 'unbalanced_tpr', 'unbalanced_prec', 'unbalanced_rec', 'unbalanced_roc', 'unbalanced_auprc', 'thresh_auc_imb', 'thresholds_auprc_imb']

perf = pd.concat([df_balanced, df_unbalanced], axis = 1)
perf.to_csv("RF_17032022_chrsplit.csv", sep = "\t")
print("done")
